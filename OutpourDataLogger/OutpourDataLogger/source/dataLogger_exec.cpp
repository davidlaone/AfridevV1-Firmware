        ...
        if (m_msgRxBytes > 0) {
            ...
            while (captureOnePacket() || m_packetCaptureReady) {
                ...
                if (m_packetCaptureReady) {
                    ...
                    if (m_packetOutpourMsgType == MSG_TYPE_DEBUG_STORAGE_INFO) {
                        if (doClearScreen) {
                            system("cls");
                            doClearScreen = false;
                        }
...
void OpLogger::copyUnitInfoFromPacketVector(void) {
    ...
    for (int i = 0; i < vectorBytes - 6; i++) {
        *stP++ = m_packetVector[i + 6];
    }
}
...
void OpLogger::printStorageData(uint64_t elapsedTime) {
	...
	uint8_t *p = (uint8_t *)&m_storageData;
    ...
    m_ConsoleBufIndex = sprintf(m_ConsoleBuffer,
                                "%10s: %06u, %10s: %06u, %10s: %06u, %10s: %06u\n", // RedFlag, RedDays, RedMapDay, Temperature
                                "RF T/F",   zeroVal,    "RF Days",    zeroVal,
                                "MAP Days", zeroVal,    "Temp C",  /* p[29] */ stdP->tempC
                               );
    // fprintf(stdout, m_ConsoleBuffer);
    m_ConsoleStringVector.push_back(m_ConsoleBuffer);
    consoleData_LogWriteData(m_ConsoleBuffer);
    ...
    m_ConsoleBufIndex = sprintf(m_ConsoleBuffer,
                                ...
                                "Align T/F", stdP->alignStorageFlag,    "Align Hour", stdP->alignHour24,
                                "Align Min",  stdP->alignMinute,        "Align Sec",  stdP->alignSecond
                               );
    // fprintf(stdout, m_ConsoleBuffer);
    m_ConsoleStringVector.push_back(m_ConsoleBuffer);
    consoleData_LogWriteData(m_ConsoleBuffer);
    ...
    m_ConsoleBufIndex = sprintf(m_ConsoleBuffer,
                                ...
                                "DailyLiter", stdP->dayMilliliterSum/1000, "Hour ML",  stdP->hourMilliliterSum, "minute ML", stdP->minuteMilliliterSum
                               );
    // fprintf(stdout, m_ConsoleBuffer);
    m_ConsoleStringVector.push_back(m_ConsoleBuffer);
    consoleData_LogWriteData(m_ConsoleBuffer);
...
void OpLogger::storageData_LogWriteData(uint64_t elapsedTime) {
    ...
    if (m_StorageDataLogFileHandle != NULL) {
        fprintf(m_StorageDataLogFileHandle,
                ...
                "%06u,%06u,%06u,%06u,"        // RedFlag, RedDays, RedMapDay, tempC
                "%06u,%06x,%06x,%06x,"        // Align T/F, AlignSec, AlighMin, AlighHour
                "%06u,%06u,%06u,%06u,%06u,%06u,%06u" // redFlagThreshTable
                "\n",

                m_LogEntryNumber,                   elapsedTime / 1000,
                stdP->dayMilliliterSum/1000,        stdP->hourMilliliterSum,         stdP->minuteMilliliterSum,
                ...
                zeroVal,                            zeroVal,
                zeroVal,                            stdP->tempC,
                stdP->alignStorageFlag,             stdP->alignSecond,
                stdP->alignMinute,
                stdP->alignHour24,
                zeroVal,
                zeroVal,
    ...
    if (status == 0) {
        ...
        if (m_StorageDataLogFileHandle) {
            ...
            fprintf(m_StorageDataLogFileHandle,
                    ...
                    "%s, %s, %s, %s,"  // RedFlag, RedDays, RedMapDay, tempC
